...
const SELECT_JOIN =
  "SELECT tw.id, tw.text, tw.userId, us.username, us.name, us.url FROM tweets as tw JOIN users as us ON tw.userId=us.id";
...
export async function getById(id) {
  return DB.execute(`${SELECT_JOIN} WHERE tw.id=?`, [id]).then(
    (result) => result[0][0]
  );
}
